...
class PointCloudHelper:
    ...
    def __init__(self, all_points_num) -> None:
        ...
        intrinsics = np.array([ [607.35,      0, 639.68],
                                [     0, 607.10, 366.60],
                                [     0,      0,     1]])
...
# 自加 抓取检测流程函数
def process_images(color_image, depth_image, pc_helper):
...
class ImageSubscriberNode:
    def __init__(self):
        ...
        # 定义订阅者
        # self.depth_sub = rospy.Subscriber('/depth_img', Ros_Image, self.depth_callback)
        # self.rgb_sub = rospy.Subscriber('/rgb_img', Ros_Image, self.rgb_callback)
    ...
    def process_images(self):
        ...
        # try:
        #     # 使用graspnet进行图像处理
        #     process_images(self.rgb_image, self.depth_image, self.pc_helper)
