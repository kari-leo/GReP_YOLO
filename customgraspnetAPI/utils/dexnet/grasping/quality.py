...
class PointGraspMetrics3D:
    ...
    def grasp_quality(grasp, obj, params, contacts=None, vis=False):
        """Computes the quality of a two-finger point grasps on a given object
        using a quasi-static model.
        ...
        if not hasattr(PointGraspMetrics3D, method):
            raise ValueError('Illegal point grasp metric %s specified' %
                             (method))
        ...
        if contacts is None:
            contacts_found, contacts = grasp.close_fingers(
                obj, check_approach=check_approach, vis=vis)
    ...
    def grasp_matrix(forces,
                     ...
                     finger_radius=0.005,
                     params=None):
        """Computes the grasp map between contact forces and wrenchs on the
        object in its reference frame.
    ...
    def force_closure_qp(forces,
                         ...
                         params=None):
        """Checks force closure by solving a quadratic program (whether or not
        zero is in the convex hull)
    ...
    def partial_closure(forces,
                        ...
                        wrench_regularizer=1e-10,
                        params=None):
        """Evalutes partial closure: whether or not the forces and torques can
        resist a specific wrench. Estimates resistance by sollving a quadratic
        ...
        for i in range(num_fingers):
            ...
            G_i = PointGraspMetrics3D.grasp_matrix(forces[:, start_i:end_i],
                                                   torques[:, start_i:end_i],
                                                   normals[:, i:i + 1],
                                                   soft_fingers,
                                                   params=params)
            G = np.c_[G, G_i]

        wrench_resisted, _ = PointGraspMetrics3D.wrench_in_positive_span(
            G,
            target_wrench,
            force_limit,
            num_fingers,
    ...
    def wrench_resistance(forces,
                          ...
                          params=None):
        """Evalutes wrench resistance: the inverse norm of the contact forces
        required to resist a target wrench Estimates resistance by sollving a
        quadratic program (min normal contact forces to produce a wrench).
        ...
        Q = 0
        wrench_resisted, finger_force_norm = PointGraspMetrics3D.wrench_in_positive_span(
            G,
            target_wrench,
            force_limit,
            num_fingers,
            ...
            wrench_regularizer=wrench_regularizer)
        if wrench_resisted:
            Q = 1.0 / (finger_force_norm +
                       finger_force_eps) - 1.0 / (2 * force_limit)
        return Q

    @staticmethod
    def min_singular(forces,
                     torques,
                     normals,
                     soft_fingers=False,
                     params=None):
    ...
    @staticmethod
    def ferrari_canny_L1_force_only(forces,
                                    torques,
                                    normals,
                                    soft_fingers=False,
                                    params=None,
                                    wrench_norm_thresh=1e-3,
                                    wrench_regularizer=1e-10):
        """Ferrari & Canny's L1 metric with force only. Also known as the
        epsilon metric.
    ...
    def min_norm_vector_in_facet(facet, wrench_regularizer=1e-10):
        """Finds the minimum norm point in the convex hull of a given facet
        (aka simplex) by solving a QP.
