...
def collision_detection(grasp_list,
                        model_list,
                        dexnet_models,
                        poses,
                        scene_points,
    ...
    **Output:**
    ...
    for i in range(num_models):
        ...
        mask1 = ((target[:, :, 2] > -height / 2) &
                 (target[:, :, 2] < height / 2))
        mask2 = ((target[:, :, 0] > -depth_base) &
                 (target[:, :, 0] < grasp_depths[:, np.newaxis]))
        mask3 = (target[:, :, 1] >
                 -(grasp_widths[:, np.newaxis] / 2 + finger_width))
        ...
        if return_dexgrasps:
            ...
            for grasp_id, _ in enumerate(grasps):
                ...
                if empty_mask[grasp_id]:
                    ...
                    continue
                center = np.array([depth, 0,
                                   0]).reshape([3, 1])  # gripper coordinate
                center = np.dot(grasp_poses[grasp_id], center).reshape([3])
                center = (center + grasp_point).reshape(
                    [1, 3])  # camera coordinate
                center = transform_points(center,
                                          np.linalg.inv(obj_pose)).reshape(
                                              [3])  # object coordinate
...
def eval_grasp(grasp_group,
    ...
    **Input:**
    ...
    if len(remain_mask) == 0:
        ...
        min_score = all_grasp_list[remain_mask[min(49,
                                                   len(remain_mask) - 1)], 0]
    ...
    for i in range(num_models):
        ...
        for grasp_id in range(num_grasps):
            ...
            score = get_grasp_score(grasp, dexnet_model, fc_list,
                                    force_closure_quality_config)
            scores.append(score)
