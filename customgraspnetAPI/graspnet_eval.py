...
from .utils.eval_utils import (create_table_points, eval_grasp, get_scene_name,
                               load_dexnet_model, parse_posevector,
                               transform_points, voxel_sample_points)
...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_scene_lr(self, dump_folder, l, r, proc=2):
        ...
        ap = np.mean(res)
        print(
            f'\nEvaluation Result:\n----------\n{self.camera}, AP {l}~{r} = {ap}'
        )
        total_grasp = 0
        colli_grasp = 0
        for scene_colli in colli:
            for image_colli in scene_colli:
                image_colli = np.array(image_colli)
                colli_grasp += image_colli.sum()
                total_grasp += len(image_colli)
        return res, ap, colli_grasp / total_grasp
    ...
    def eval_seen(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        '''
        res = np.array(
            self.parallel_eval_scenes(scene_ids=list(range(100, 130)),
                                      dump_folder=dump_folder,
                                      proc=proc))
        ap = np.mean(res)
        print('\nEvaluation Result:\n----------\n{}, AP Seen={}'.format(
    ...
    def eval_similar(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        '''
        res = np.array(
            self.parallel_eval_scenes(scene_ids=list(range(130, 160)),
                                      dump_folder=dump_folder,
                                      proc=proc))
        ap = np.mean(res)
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Similar={}'.
              format(self.camera, ap, ap))
        return res, ap
    ...
    def eval_novel(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        '''
        res = np.array(
            self.parallel_eval_scenes(scene_ids=list(range(160, 190)),
                                      dump_folder=dump_folder,
                                      proc=proc))
        ap = np.mean(res)
        print(
    ...
    def eval_all(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        '''
        res = np.array(
            self.parallel_eval_scenes(scene_ids=list(range(100, 190)),
                                      dump_folder=dump_folder,
                                      proc=proc))
        ap = [
            np.mean(res),
        ...
        print(
            '\nEvaluation Result:\n----------\n{}, AP={}, AP Seen={}, AP Similar={}, AP Novel={}'
            .format(self.camera, ap[0], ap[1], ap[2], ap[3]))
        return res, ap
