...
from models.Physical_Constraint_Regularization import *
from models.anchornet import AnchorGraspNet, BNMomentumScheduler
from models.localgraspnet import PointMultiGraspNet
from models.pointnet import PointNetwide
...
def validate(epoch, anchornet: nn.Module, localnet: nn.Module, widenet: nn.Module,
    ...
    with torch.no_grad():
        for anchor_data, rgb, depth, grasppaths in tqdm(val_data,
            ...
            if epoch >= args.pre_epochs:
                ...
                rect_ggs = [rect_gg]
                pc_group, wide_pc_group,  valid_local_centers = data_process(
                    points_all,
                    depth.cuda(),
                    rect_ggs,
                    args.center_num,
                ...
                # 获取wide特征
                wide_features = widenet(wide_pc_group) #得到(total_center_num, feature_dim)
                # 拼接wide特征和pc_group
                # print(wide_features.shape)
                # 扩展 wide_features 的维度，使其能拼接
                # 先加一个维度： [B, 1, F]
                wide_features_expanded = wide_features.unsqueeze(1)  # [B, 1, F]

                # 再重复 N 次： [B, N, F]
                wide_features_repeated = wide_features_expanded.repeat(1, pc_group.size(1), 1)

                # 拼接： [B, N, C+F]
                pc_group = torch.cat([pc_group, wide_features_repeated], dim=-1)
                ...
                # PCR
                # print('pred_view shape: ', pred_view.shape)
                # print('pred_rect_gg shape: ', len(pred_rect_gg))
                # Physucal condtraint computation
                pred_grasp_from_rect = pred_rect_gg.to_6d_grasp_group()
                PCR = PC_Regularization(points_all, pred_grasp_from_rect)
    ...
    results['PCR'] /= batch_idx
    for ln, l in anchor_losses.items():
        results['losses'][ln] /= batch_idx
    return results


def train(epoch, anchornet: nn.Module, localnet: nn.Module, widenet:nn.Module, 
...
def run():
    ...
    tb, save_folder = prepare_torch_and_logger(args, is_wide=True)
    ...
    anchornet = AnchorGraspNet(ratio=args.ratio,
                               ...
                               anchor_k=args.anchor_k)
    localnet = PointMultiGraspNet(3, args.anchor_num**2, extra_feature_len=32+32)
    widenet = PointNetwide(feature_len=3)
    ...
    if args.checkpoint is not None:
        ...
        # widenet.load_state_dict(ckpt['wide'])

    # set optimizer
    params = itertools.chain(anchornet.parameters(), localnet.parameters(), widenet.parameters())
    ...
    widenet = nn.parallel.DataParallel(widenet).cuda()
    logging.info('Done')

    for epoch in range(args.epochs):
        logging.info('Beginning Epoch {:02d}'.format(epoch))
        train_results = train(epoch, anchornet, localnet, widenet, train_data,
        ...
        if epoch >= args.pre_epochs:
            ...
            tb.add_scalar('train_loss/PCR',
                          train_results['PCR'], epoch)

        # Run Validation
        logging.info('Validating...')
        val_results = validate(epoch, anchornet, localnet, widenet, val_data, anchors,
                               args)
