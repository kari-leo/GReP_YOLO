...
from dataset.evaluation import (anchor_output_process, collision_detect,
                                detect_2d_grasp_yolo, detect_6d_grasp_multi)
...
def inference(view_points,
    ...
    with torch.no_grad():
        ...
        rect_gg = detect_2d_grasp_yolo(loc_map,
                                  cls_mask,
                                  theta_offset,
                                  height_offset,
                                  width_offset,
...
if __name__ == '__main__':
    ...
    widenet.load_state_dict(check_point['wide'])
    # load checkpoint
    ...
    # YOLO
    # Load a model
    model = YOLO("yolov8/yolo11n-seg.pt")  # pretrained YOLO11n model
    # Run batched inference on a list of images
    bgr_yolo = np.array(Image.open(args.rgb_path))[:, :, ::-1] 
    results = model(bgr_yolo)  # return a list of Results objects]
    for result in results:
        boxes = result.boxes  # Boxes object for bounding box outputs
        masks = result.masks  # Masks object for segmentation masks outputs
        ori_shape = result.masks.orig_shape
        result.show()  # display to screen
    for box, mask in zip(boxes, masks):
        mask = mask.data.cpu().numpy().transpose(1, 2, 0) * 255
        # print("befroe scaling", mask.shape)
        mask = ops.scale_image(mask, ori_shape)
        #将mask中的值转换为0和置信度
        obj_mask = np.squeeze(np.where(mask>0, box.conf.cpu()[0], 0))
        #将mask转换为符合本次运行要求的形状
        obj_mask = torch.from_numpy(obj_mask).unsqueeze(0).unsqueeze(0) # 转为 (1, 1, H, W)
        obj_mask = F.interpolate(obj_mask, (args.input_h, args.input_w)).squeeze().T #插值
        # print(obj_mask.shape)
        plt.figure()
        plt.imshow(Image.open(args.rgb_path))
